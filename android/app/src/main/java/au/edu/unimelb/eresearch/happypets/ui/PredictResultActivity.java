...
import au.edu.unimelb.eresearch.happypets.adapters.PredictionRVAdapter;
...
import au.edu.unimelb.eresearch.happypets.utils.PlayStoreUtils;
import au.edu.unimelb.eresearch.happypets.utils.SharedPref;
...
public class PredictResultActivity extends BaseActivity implements PredictionRVAdapter.PredictFeedbackListener {
    ...
    @Override
    public void updatePredictionFeedback(int id, boolean breedCorrect, boolean emotionCorrect) {
        for (int i = 0; i < this.predictions.size(); i++) {
            Prediction prediction = this.predictions.get(i);
            if (prediction.getId() == id) {
                prediction.setBreedFeedbackCorrect(breedCorrect);
